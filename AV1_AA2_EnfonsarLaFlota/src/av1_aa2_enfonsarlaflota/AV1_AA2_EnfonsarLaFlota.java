...
public class AV1_AA2_EnfonsarLaFlota {
    ...
    private static void jugar_partida(int files, int columnes, int llanxes,
        ...
        while (intents < maxIntents && !veureTot) {
            mostra_tauler(tauler, false); // Mostra el tauler sense revelar la posició dels barcos
    ...
    private static int damana_dades_entre_max_min(String dades, int min, int max) {
        ...
        while (true) {
            ...
            while (!scanner.hasNextInt()) {
                System.out.println("Això no és un número. Intenta-ho de nou:");
                scanner.next();
                System.out.print(dades);
            }
            ...
            if (valor >= min && valor <= max) {
                ...
                System.out.println("El número ha de ser entre " + min + " i " + max + ".");
            }
        }
    }
/**
 * FUNCTION: Funció que recorre el tauler buscant caselles amb lletra corresponent
 * a vaixell. 
 * @param tauler
